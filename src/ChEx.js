 ...
 * @param opt.title
 */
ChEx.templateStorage2 = function (opt) {
    const $table = $('<table></table>');
    const $root = $('<div></div>').append(
        $table,
    ...
    const __addRow = function (text, value) {
        let $tr = $('<tr></tr>');
        $tr.append(
            ...
            $('<td style="padding: 0 4px;"></td>').text(text.replace(/\t[\s\S]*$/, '')).attr('title', text),
            $('<td><input type="button" value="Delete"></td>').click(() => {
                if (!confirm(`Delete?\n\n${text.replace(/\n/g, ' ').replace(/\t/g, "\n")}`)) return;
                $tr.remove();
                ChEx.storage.saveLocal(opt.storageKey, [], function (templates) {
                    let idx = templates.indexOf(value);
                    if (idx === -1) return ChEx.error('No value in the template. value: ' + value);
                    templates.splice(idx, 1);
                });
            }),
        );
